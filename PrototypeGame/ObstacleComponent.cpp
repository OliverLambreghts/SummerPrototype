...
std::shared_ptr<GameObject> ObstacleComponent::Clone()
{
	...
	clone->AddComponent(std::make_shared<ObstacleComponent>(m_IsBreakable, m_FileName, m_Position, m_pPlayer, m_pEnemies));
...
std::vector<std::shared_ptr<GameObject>> ObstacleComponent::GetEnemies() const
{
	return m_pEnemies;
}
